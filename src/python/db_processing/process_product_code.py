...
def process_product_code(db_path: str = 'data/processed/psmv-data.duckdb'):
    ...
    SELECT 
        ...
        c.IT
    FROM (
        SELECT LOWER(TRIM(product_ref_or_id)) AS product_ref_or_id, LOWER(TRIM(code_id)) AS code_id FROM ProductCodeR
        WHERE product_ref_or_id IS NOT NULL AND code_id IS NOT NULL

        UNION ALL

        SELECT LOWER(TRIM(product_ref_or_id)) AS product_ref_or_id, LOWER(TRIM(code_id)) AS code_id FROM ProductCodeS
        WHERE product_ref_or_id IS NOT NULL AND code_id IS NOT NULL

        UNION ALL

        SELECT LOWER(TRIM(product_ref_or_id)) AS product_ref_or_id, LOWER(TRIM(code_id)) AS code_id FROM ProductDangerSymbol
        WHERE product_ref_or_id IS NOT NULL AND code_id IS NOT NULL

        UNION ALL

        SELECT LOWER(TRIM(product_ref_or_id)) AS product_ref_or_id, LOWER(TRIM(code_id)) AS code_id FROM ProductSignalWords
        WHERE product_ref_or_id IS NOT NULL AND code_id IS NOT NULL
    ) p
    LEFT JOIN (
        SELECT 
            LOWER(TRIM(code_id)) AS code_id,
            text_key,
        ...
        FROM Code
        GROUP BY LOWER(TRIM(code_id)), text_key
    ) c
