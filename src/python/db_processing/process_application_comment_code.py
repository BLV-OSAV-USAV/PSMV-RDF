...
def process_application_comment_code(db_path: str = 'data/processed/psmv-data.duckdb'):
    con = duckdb.connect(db_path)

    con.execute("""
        CREATE OR REPLACE TABLE ApplicationCommentCode AS
        SELECT 
            code_id,
    ...
    row_count = con.execute("SELECT COUNT(*) FROM ApplicationCommentCode").fetchone()[0]
    print(f"[i] ApplicationCommentCode created with {row_count} rows.")
